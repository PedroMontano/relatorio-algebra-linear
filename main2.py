...
from linear_methods import cramer
import plot
...
f = plot.line_plot( x_list, y_list, labels, x_label = 'Ordem da Matriz', y_label = 'Tempo de Processamento [ log10 ]', title = 'Tempo x Dimensão', xtick_format = int, colors = list( _COLORS.values( ) ), marker = 'o' )
f.savefig( _FIGURE_FOLDER + '4_tempo_dimensao.png' )


_REGRESSION = { }
for i, m in enumerate( _MATRIX_NAMES ):
    slope, intercept, r_value, p_value, stderr = linregress( x_list[ i ][ 4 : ], y_list[ i ][ 4 : ] )
    _REGRESSION[ m ] = ( slope, intercept )

a = _REGRESSION[ 'TOEPLITZ' ][ 0 ]
b = _REGRESSION[ 'TOEPLITZ' ][ 1 ]
time0 =  ( 10 ** ( a * 15 + b ) ) / ( 3600 * 24 )
...
f = plot.bar_plot( height_list = height_list, labels = _MATRIX_NAMES, y_label = 'Erro Absoluto Médio [ log10 ]', colors = list( _COLORS.values( ) ), title = 'Erro - Matriz 8x8' )
...
x_list = [ _DIMENSIONS for _ in range( 4 ) ]
f = plot.line_plot( x_list, y_list, labels, x_label = 'Ordem da Matriz', y_label = 'Erro Absoluto Médio [ log10 ]', title = 'Erro x Dimensão', xtick_format = int, colors = list( _COLORS.values( ) ), marker = 'o' )
